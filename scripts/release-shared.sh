#!/bin/bash
# Release script for @itsaydrian/waymark-shared
# Usage: ./scripts/release-shared.sh <version>
# Example: ./scripts/release-shared.sh 0.0.2
...
if [ ! -f "dist/types/index.js" ]; then
  echo "Error: dist/types/index.js not found. Build failed."
  exit 1
fi
if [ ! -f "dist/lib/index.js" ]; then
  echo "Error: dist/lib/index.js not found. Build failed."
  exit 1
fi
echo "✓ Build verified - output files present"
...
echo "  3. git commit -m \"chore(shared): release v$VERSION\""
echo "  4. git tag \"@itsaydrian/waymark-shared@$VERSION\""
echo "  5. git push origin \"@itsaydrian/waymark-shared@$VERSION\""
echo "  6. gh release create \"@itsaydrian/waymark-shared@$VERSION\" --title \"@itsaydrian/waymark-shared@$VERSION\" --notes \"...\" --repo aydrian/waymark"
